...
class MeasuresAssociation():    
    ...
    #----------------------------------------#
    # MI поднимает наверх сочетания с редкими словами 
    # (т.е чувствительна к низкочастотным словам), опечатками, 
    # иностранными словами -> необходим порог снизу по частотности
    # МI описывает предметную область: выделяются имена собственные, 
    # специальные термины,сложные номинации, отражающие предметную область
    ...
    def pearson(cn_freq,n_freq,c_freq,total):
        ...
        # cn_freq -  частота совместной встречаемости ключевого слова в паре 
        # с коллокатом: то есть это число повторов биграммы в списке всех биграмм
    ...
    #----------------------------------------#
    # Эвристическая мера MI3 увеличивает вес частоты совместной встречаемости 
    # в числителе, что не даёт MI завышать значения для низкочастотных сочетаний. 
    # Таким образом, MI3 должна показать лучшие результаты при выявлении 
    ...
    # Эта мера увеличивает вес частоты совместной встречаемости 
    # ключевого слова и коллоката по сравнению с MI. 
